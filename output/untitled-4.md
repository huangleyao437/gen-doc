*   [English](/docs/4.x/getting-started/before-you-start-the-poc/)
...
写入时按 SUM/MAX/MIN 合并行

**Duplicate Key 适用于大多数场景。**详见[数据模型概述](/zh-CN/docs/4.x/getting-started/table-design/data-model/intro/)。

### Sort Key（排序键）[​](#sort-key排序键 "Sort Key（排序键）的直接链接")

Doris 在排序键的前 36 字节上构建[前缀索引](/zh-CN/docs/4.x/getting-started/table-design/index/prefix-index/)，设置排序键时注意以下原则：

*   **高频过滤列优先**：将最常用于 WHERE 条件的列放在最前面。
*   **定长类型优先**：INT、BIGINT、DATE 等定长类型放在 VARCHAR 之前，因为遇到 VARCHAR 时前缀索引会立即截断。
*   **补充倒排索引**：前缀索引覆盖不到的列，可添加[倒排索引](/zh-CN/docs/4.x/getting-started/table-design/index/inverted-index/overview/)加速过滤。

### 分区[​](#分区 "分区的直接链接")

如果有时间列，使用 `AUTO PARTITION BY RANGE(date_trunc(time_col, 'day'))` 启用[分区裁剪](/zh-CN/docs/4.x/getting-started/table-design/data-partitioning/auto-partitioning/)。Doris 会自动跳过无关分区。

### 分桶[​](#分桶 "分桶的直接链接")

默认是 **Random 分桶**（推荐用于 Duplicate Key 表）。如果频繁按某列过滤或 JOIN，使用 `DISTRIBUTED BY HASH(该列)`。详见[数据分桶](/zh-CN/docs/4.x/getting-started/table-design/data-partitioning/data-bucketing/)。
...
确保数据均匀分布。后续扩容 BE 时，查询通常涉及多个分区，性能不会受影响
...
Unique Key 表 ≤ 10 GB。可通过 `SHOW TABLETS FROM your_table` 查看

每个分区不超过 128 个分桶

需要更多时优先考虑增加分区。极端情况下上限为 1024，但生产环境中很少需要

### 建表模板[​](#建表模板 "建表模板的直接链接")

#### 日志 / 事件分析[​](#日志--事件分析 "日志 / 事件分析的直接链接")
...
*   **批量数据不要用 `INSERT INTO VALUES`**。请使用 [Stream Load](/zh-CN/docs/4.x/getting-started/data-operate/import/import-way/stream-load-manual/) 或 [Broker Load](/zh-CN/docs/4.x/getting-started/data-operate/import/import-way/broker-load-manual/)。详见[导入概述](/zh-CN/docs/4.x/getting-started/data-operate/import/load-manual/)。
*   **优先在客户端合并写入。**高频小批次导入导致版本堆积。如不可行，使用 [Group Commit](/zh-CN/docs/4.x/getting-started/data-operate/import/load-best-practices/group-commit-manual/)。
*   **将大型导入拆分为小批次。**长时间运行的导入失败后必须从头重试。使用 [INSERT INTO SELECT 配合 S3 TVF](/zh-CN/docs/4.x/getting-started/data-operate/import/import-way/streaming-job/continuous-load-overview/) 实现增量导入。
...
*   **正确设置排序键。**与 PostgreSQL 等系统不同，Doris 仅对排序键的前 36 字节建立索引，且遇到 VARCHAR 会立即截断。超出前缀范围的列无法从排序键受益，需添加[倒排索引](/zh-CN/docs/4.x/getting-started/table-design/index/inverted-index/overview/)。参见 [Sort Key（排序键）](#sort-key%E6%8E%92%E5%BA%8F%E9%94%AE)。
...
诊断慢查询请使用 [Query Profile](/zh-CN/docs/4.x/getting-started/query-acceleration/query-profile/)。
...
### 确保分区裁剪生效[​](#确保分区裁剪生效 "确保分区裁剪生效的直接链接")
...
### 开启 Data Cache[​](#开启-data-cache "开启 Data Cache的直接链接")

远端存储（HDFS/对象存储）的 IO 延迟比本地磁盘高出数倍。Data Cache 将最近访问的远端数据缓存到 BE 本地磁盘，**重复查询同一批数据时可获得接近内表的查询性能**。

*   缓存默认关闭，请参阅 [数据缓存](/zh-CN/docs/4.x/getting-started/lakehouse/data-cache/) 文档进行配置。
...
### 使用 Query Profile 诊断[​](#使用-query-profile-诊断 "使用 Query Profile 诊断的直接链接")

湖上查询的瓶颈通常在 IO 而非计算。[Query Profile](/zh-CN/docs/4.x/getting-started/query-acceleration/query-profile/) 可以定位慢查询根因，重点关注：
...
更多优化手段请参阅[数据湖查询调优](/zh-CN/docs/4.x/getting-started/lakehouse/best-practices/optimization/)。

## 常见报错与解决方案[​](#常见报错与解决方案 "常见报错与解决方案的直接链接")

### 建表报错 "Tablet count should be greater than 0"[​](#建表报错-tablet-count-should-be-greater-than-0 "建表报错 \"Tablet count should be greater than 0\"的直接链接")
...
### 导入版本堆积，查询变慢[​](#导入版本堆积查询变慢 "导入版本堆积，查询变慢的直接链接")
...
### Q: POC 验证需要多久？[​](#q-poc-验证需要多久 "Q: POC 验证需要多久？的直接链接")

一般 1-2 天完成基础功能验证（建表、导入、简单查询），详细性能调优需要 3-5 天。

### Q: 建表时如何选择分桶数？[​](#q-建表时如何选择分桶数 "Q: 建表时如何选择分桶数？的直接链接")

优先设为 BE 节点数的整数倍，确保数据均匀分布。每个分桶压缩后数据建议 ≤ 20 GB（Unique Key 表 ≤ 10 GB）。

### Q: 查询比预期慢怎么办？[​](#q-查询比预期慢怎么办 "Q: 查询比预期慢怎么办？的直接链接")

1.  执行 `EXPLAIN` 检查是否走索引
2.  执行 `SHOW TABLETS` 检查数据是否倾斜
3.  查看 Query Profile 定位瓶颈

### Q: Data Cache 是否需要开启？[​](#q-data-cache-是否需要开启 "Q: Data Cache 是否需要开启？的直接链接")

如果涉及湖上查询（Hive/Iceberg/Paimon），建议开启。首次查询会自动缓存数据，重复查询可获得接近内表的性能。

**标签：**

*   [POC](/zh-CN/docs/4.x/tags/poc/)
*   [建表](/zh-CN/docs/4.x/tags/建表/)
